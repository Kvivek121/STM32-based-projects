...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
unsigned char rx_global[1];
//uint8_t *SPI_Read_Register( uint8_t reg_adr);
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    /* USER CODE END WHILE */
  int temp;
  	temp=  readRegister(0xd0);
  	  writeRegister(0xf5,0x20);
  	  readRegister(0xf5);
...
uint8_t *SPI_Read_Register( uint8_t reg_adr)
{
	...
	HAL_SPI_TransmitReceive(&hspi1, tx, rx, 1, (uint32_t)1000);
	HAL_SPI_TransmitReceive(&hspi1, 0, rx, 1, (uint32_t)1000);
